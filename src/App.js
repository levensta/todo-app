...
// более простое решение, нежели писать много строчек с fetch
...
function App() {
	...
	const onEditTask = (listId, taskObj) => {
		...
		if (newTaskText) {
			...
			setLists(newLists);
			axios.patch("http://localhost:8000/tasks/" + taskObj.id, {
				text: newTaskText
			})
			.catch(() => {
	...
	const onRemoveTask = (listId, taskId) => {
		if (window.confirm("Вы действительно хотите удалить задачу?")) {
			...
			setLists(newLists);
			axios.delete("http://localhost:8000/tasks/" + taskId).catch(() => {
		...
		<div className="todo">
			<div className="todo__sidebar">
				...
				{ lists ?
					(<List
						items={lists}
						// после удаления элемента из бэкенда, нужно обновить стейт
						// и пересоздать список, убрав удаленный элемент
						onRemove={id => {
							const newLists = lists.filter(item => item.id !== id);
							setLists(newLists);
						}}
						onClickItem={item => {
							// react-router less v6
